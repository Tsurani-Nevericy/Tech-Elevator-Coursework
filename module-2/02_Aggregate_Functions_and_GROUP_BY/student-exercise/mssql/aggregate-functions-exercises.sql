...
select avg(lifeexpectancy) as average_life_expectancy_in_SA
from country
where continent = 'South America';

-- 5. The sum of the population of all cities in California.
-- (total population of all cities in California: 16716706)
select sum(population) as sum_population_in_Cali
from city
where district = 'California';

-- 6. The sum of the population of all cities in China.
-- (total population of all cities in China: 175953614)
select sum(population) as sum_pop_in_China
from city
...
select avg(population) as avg_us_city_pop
from city
where countrycode = 'USA';

-- 12. The average population of cities in China.
-- (average city population in China: 484720.6997 approx.)
select avg(population) as avg_china_city_pop
from city
where countrycode = 'CHN';
...
select top 10 name, (population/surfacearea) as pop_density, lifeexpectancy
from country
where lifeexpectancy is not null and surfacearea > 0
...
select (gnp-gnpold) as gnp_diff, abs(gnpold-gnp) as abs_gnp_diff, name
from country
...
select count(name) as num_countries, continent
from country
group by continent
...
select max(population) as largest_pop_city_in_country, countrycode
from city
group by countrycode
order by largest_pop_city_in_country desc;
