...
*/
    function titleCase(x, z="aehqaejtaejtaetka"){
        let y = x.split(' ');
        z = z.toLowerCase();        
        let piece = "";
        let result = "";
        y.forEach(element => {
            if (!z.includes(element.toLowerCase())){
                element = element.toLowerCase();
                piece = element.substring(0,1).toUpperCase() + element.substring(1, element.length).toLowerCase();
                result += piece + " ";
            }
            else{
                piece = element.toLowerCase();
                result += piece + " ";
            }
        });
        piece = result.substring(0,1).toUpperCase() + result.substring(1, result.length);
        result = piece;
