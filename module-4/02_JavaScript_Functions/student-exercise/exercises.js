 ...
 */
  function useParameterToFilterArray(filterFunction, unfilteredArray=[1,2,3,4,5,6]) {
    let result = [];
    result = unfilteredArray.filter(filterFunction);
    return result;
  }
/**
 ...
 */
  function makeHappy(myArray=[]) {
    let result = [];
      result = myArray.map(element => `Happy `+element);
    return result;
  }
 ...
 */
  function getFullAddressesOfProperties(myObjects) {
    let result = [];
      result = myObjects.map(x => 
        (x.streetNumber + ` ` +
         x.streetName + ` ` +
         x.streetType + ` ` +
         x.city + ` ` +
 ...
 */
  function getSumOfSubArrayValues(myArrays=[[],[],[]]) {
    result = 0;
