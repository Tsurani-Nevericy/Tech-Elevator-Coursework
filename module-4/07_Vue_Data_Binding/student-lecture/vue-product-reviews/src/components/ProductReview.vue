<template>
    <div class="main">
        <button v-on:click="changeInline">Change Inline</button>
        <button v-on:click="displayName">{{nameBtnTxt}}</button>
        <h2 v-show="showName" v-bind:class="{ 'display-inline': isInline, 'display-color': !isInline }">Product Reviews for {{ name }}</h2>
        
        <button v-on:click="displayDescription">{{descriptionBtnTxt}}</button>
...
export default {
    ...
    methods: {
        getNumberOfReviewsForRating(reviews, rating) {
            return reviews.reduce((currentCount, review)=>{
                return review.rating === rating ? ++currentCount : currentCount;
            }, 0);
        },
        displayName() {
    ...
    computed: {
        ...
        numberOfOneStarReviews(vm) {
            return vm.getNumberOfReviewsForRating(vm.reviews, 1);
        },
        numberOfTwoStarReviews(vm) {
            return vm.getNumberOfReviewsForRating(vm.reviews, 2);
        },
        numberOfThreeStarReviews(vm) {
            return vm.getNumberOfReviewsForRating(vm.reviews, 3);
        },
        numberOfFourStarReviews(vm) {
            return vm.getNumberOfReviewsForRating(vm.reviews, 4);
        },
        numberOfFiveStarReviews(vm) {
            return vm.getNumberOfReviewsForRating(vm.reviews, 5);
        },
        nameBtnTxt(vm) {
            return vm.showName ? "Hide Name" : "Show Name";
        },
        descriptionBtnTxt(vm) {
            return vm.showDescription ? "Hide Description" : "Show Description";
        }
    }
}
</script>
